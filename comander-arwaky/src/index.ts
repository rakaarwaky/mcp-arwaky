...
import { startUnixSocketServerIfNeeded } from './unix-socket-server.js';
...
async function runServer() {
  try {
    ...
    const socketMode = process.argv.includes('--socket-mode');
    const socketOnly = process.argv.includes('--socket-only');

    // When --socket-mode --socket-only: skip MCP stdio, just run socket server
    // This allows running as standalone socket server without MCP client on stdin
    if (socketMode && socketOnly) {
      logToStderr('info', 'Running in socket-only mode (no MCP stdio)');

      const socketServer = await startUnixSocketServerIfNeeded(process.argv);
      if (socketServer) {
        logToStderr('info', `Unix Socket Server started on ${socketServer.getSocketPath()}`);
        logToStderr('info', 'Socket server is ready to accept connections');
      }
      ...
      await new Promise<void>((resolve) => {
        ...
        // Also keep alive if stdin closes
