...
var (
	...
	ToolsetMetadataHydra = inventory.ToolsetMetadata{
		ID:          "hydra",
		Description: "Meta-tools that aggregate multiple operations for efficiency",
		Icon:        "workflow",
	}
)
...
func AllTools(t translations.TranslationHelperFunc) []inventory.ServerTool {
	return []inventory.ServerTool{
		// --- Hydra Meta-Tools (4) ---
		GithubListActivity(t),      // Aggregates notifications, issues, PRs
		GithubUnderstandCodebase(t), // Tree + Search
		GithubExecuteWorkflow(t),    // Multi-step (feature_start)
		GithubManageThread(t),       // Full conversation context

		// --- Essential Granular Tools (14) ---
		GetMe(t),              // Context/Auth check
		SearchRepositories(t), // Repo discovery
		SearchCode(t),         // Exact pattern search
		SearchRepositories(t), // Repository discovery
		GetFileContents(t),    // Reading files
		ListCommits(t),        // History
		ListBranches(t),       // Branch management
		CreateBranch(t),       // Branch creation
		PushFiles(t),          // Committing code
		CreateOrUpdateFile(t), // Single file update
		DeleteFile(t),         // File removal
		CreateRepository(t),   // New projects
		CreatePullRequest(t),  // PR creation
		MergePullRequest(t),   // Merging
		PullRequestRead(t),    // PR details
		IssueWrite(t),         // Issue management
		AddIssueComment(t),    // Quick comments
	}
}
