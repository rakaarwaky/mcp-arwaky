...
async function loadCredentials() {
    try {
        ...
        if (fs.existsSync(OAUTH_PATH)) {
            ...
            keys = keysContent.installed || keysContent.web;
        }
        ...
        if (!fs.existsSync(OAUTH_PATH) || !keys) {
            if (!fs.existsSync(OAUTH_PATH)) {
                console.error('Warning: OAuth keys file not found. Please place gcp-oauth.keys.json in current directory or', CONFIG_DIR);
            } else {
                console.error('Error: Invalid OAuth keys file format. File should contain either "installed" or "web" credentials.');
            }
            console.error('Creating dummy client to allow server to start without valid OAuth credentials.');
        ...
        console.error('Creating dummy client to allow server to start.');
        oauth2Client = new OAuth2Client(
            'dummy-client-id',
            'dummy-client-secret',
            'http://localhost:3000/oauth2callback'
